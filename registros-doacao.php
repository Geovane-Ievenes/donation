...
class PDF extends FPDF{
    ...
    function ImprovedTable($header, $data){
        ...
        $this->Write(1.5,'Comprovante de doações do usuário');
        $this->SetXY(8.7 , 5);
        $this->SetFont('Helvetica','I',12);
        $this->Write(1.5, $DOADOR);
        $this->Ln(2);
        ...
        foreach($data as $doacao){
            ...
            $this->SetFont('','B', 7);
            $this->Cell($w[6],2,'Link do comprovante','LR',0,'C',$fill, 'http://localhost/donation2.6/comprovantes/download.php?document='.$doacao['comprovante']);
            /*Reset Color*/ 
            $this->SetTextColor(0, 0, 0);
            $this->SetFont('','B', 10);
            $this->Ln(); 
